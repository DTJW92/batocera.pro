...
APPLINK=$(curl -s https://api.github.com/repos/th-ch/youtube-music/releases/latest | grep "browser_download_url.64.*AppImage" | cut -d '"' -f 4)
...
cd $temp
curl --progress-bar -L -o "$temp/youtube-music.AppImage" "$APPLINK"
cd ~/
