...
APPLINK="$(curl -s https://api.github.com/repos/cosmo0/arcade-manager/releases/latest | jq -r ".assets[] | select(.name | endswith(\"x64.AppImage\")) | .browser_download_url")"
...
cd $temp
curl --progress-bar -L -o "$temp/ArcadeManager.sh" "$APPLINK"
cd ~/
